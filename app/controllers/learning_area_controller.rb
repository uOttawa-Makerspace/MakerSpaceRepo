class LearningAreaController < DevelopmentProgramsController
  ...
  def create
    ...
    if @learning_module.save
      begin
        ...
        render json: {redirect_uri: learning_area_path(@learning_module.id)
      end
    else
  ...
  private
    ...
    def learning_modules_params
      params.require(:learning_module).permit(:title, :description, :training_id, :level, :cc, :badge_template_id)
    end
