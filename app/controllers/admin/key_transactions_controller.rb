class Admin::KeyTransactionsController < AdminAreaController
  ...
  def update
    if !params[:key_transaction][:deposit_return_date].empty? &&
      ...
      redirect_to admin_key_transactions_path, alert: "Something went wrong while updating the deposit"
    end
  end
  ...
  def set_key_transaction
    ...
    if @key_transaction.nil?
      redirect_to admin_key_transactions_path, alert: "Key Transaction was not found"
    end
  end
