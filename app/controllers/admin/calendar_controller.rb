class Admin::CalendarController < AdminAreaController
  ...
  def shifts_json
    space_id = params[:space_id].present? ? params[:space_id].to_i : (@user.space_id || Space.first.id)
  ...
  def get_user_color(user)
    ...
    space_id = params[:space_id].presence || @user.space_id || Space.first.id
    StaffSpace.find_by(user_id: user.id, space_id: space_id)&.color.presence || "#000000"
  end
