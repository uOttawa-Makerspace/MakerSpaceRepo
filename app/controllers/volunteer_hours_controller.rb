...
class VolunteerHoursController < VolunteersController
  ...
  before_action :validate_staff_for_request, only: %i[volunteer_hour_requests volunteer_hour_per_user]
  ...
  def volunteer_hour_per_user
    ...
    @users_with_volunteer_hours = User.volunteers.joins(:volunteer_hours).distinct.paginate(page: params[:page], per_page: 15)
