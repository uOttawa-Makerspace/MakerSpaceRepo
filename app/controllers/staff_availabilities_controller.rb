class StaffAvailabilitiesController < StaffAreaController
  ...
  def create
    ...
    respond_to do |format|
      if @staff_availability.save
        ...
        format.html { render :new }
        format.json { render json: @staff_availability.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def check_admin_or_staff_in_space
    unless @user.admin? || @user.staff_in_space?
      ...
      flash[:alert] = 'You cannot access this area. If you think you should be able, try asking your manager if you were added in one of the spaces.'
    end
  end
