...
class PrintOrdersController < ApplicationController
  ...
  def index
    @order = {
      ...
      "user_approval is NULL and approved is TRUE" =>
        "Waiting for user Approval",
      "user_approval is TRUE and approved is TRUE and printed is NULL" =>
        "Waiting to be printed",
      "user_approval is TRUE and approved is TRUE and printed is TRUE" =>
        "Printed",
    ...
    @print_order =
      if @user.staff? || @user.admin?
        PrintOrder
          ...
          .order(expedited: :desc, created_at: :asc)
          .includes(
            :user,
            file_attachment: :blob,
            final_file_attachments: :blob,
            pdf_form_attachment: :blob
          )
      else
        @user
          .print_orders
          .filter_by_date(params[:start_date], params[:end_date])
          .order(expedited: :desc, created_at: :desc)
          .includes(
            :user,
            file_attachment: :blob,
            final_file_attachments: :blob,
  ...
  def index_new
    @order = {
      ...
      "approved is NULL" => [
        "Waiting on Admin's approval",
        ["Waiting on your approval", "Queued to be printed", "Printed"]
      ],
      "user_approval is NULL and approved is TRUE" => [
        "Approved by Admins",
        "Waiting on your approval",
        ["Queued to be printed", "Printed"]
      ],
      "user_approval is TRUE and approved is TRUE and staff_id is NULL and printed is NULL" => [
        "Approved by Admins",
        "Approved by you",
        "Queued to be printed",
        "Currently being printed"
      ],
      "user_approval is TRUE and approved is TRUE and staff_id is NOT NULL and printed is NULL" => [
        "Approved by Admins",
        "Approved by you",
        "Queue is done",
        "Currently being printed",
      ...
      "user_approval is TRUE and approved is TRUE and staff_id is NOT NULL and printed is TRUE and updated_at > NOW() - INTERVAL '7 days'" => [
        "Approved by Admins",
        "Approved by you",
        "Queue is done",
        "Printed",
      ...
      "approved is FALSE and updated_at > NOW() - INTERVAL '7 days'" => [
        "Declined by admins",
        ""
      ],
      "user_approval is FALSE and updated_at > NOW() - INTERVAL '7 days'" => [
        "Approved by admins",
        "Declined by you",
        ""
      ]
  ...
  def update_submission
    ...
    if @print_order.update(print_order_params)
      if @print_order.approved?
        MsrMailer.send_print_quote(
          $expedited_price,
          @print_order.user,
          @print_order,
          params[:print_order][:staff_comments],
          $clean_part_price,
          true
        ).deliver_now
      end
  ...
  def update
    ...
    params[:print_order][:timestamp_approved] = DateTime.now if params[
      :print_order
    ][
      :timestamp_approved
    ]
    params[:print_order][:timestamp_printed] = DateTime.now if params[
      :print_order
    ][
      :printed
    ]
    ...
    if @print_order.update(print_order_params)
      if params[:print_order][:approved] == "true"
        ...
        MsrMailer.send_print_declined(
          @print_order.user,
          params[:print_order][:staff_comments],
          @print_order.file.filename
        ).deliver_now
      elsif params[:print_order][:user_approval] == "true"
        MsrMailer.send_print_user_approval_to_makerspace(
          @print_order.id
        ).deliver_now
        ...
        if params[:email_false].blank?
          if params[:email_message].present?
            ...
            message =
              "<div>Hi #{@print_order.user.name}, <br>Your print has completed and is now available for pickup! Your order ID is #{@print_order.id} and your quoted balance is $. Please visit <a href='https://wiki.makerepo.com/wiki/How_to_pay_for_an_Order'>https://wiki.makerepo.com/wiki/How_to_pay_for_an_Order</a> for information on how to pay for your job and email makerspace@uottawa.ca to arrange for the pick up of your part during weekdays between 9h-17h.<br><br></div><div>Best regards,<br>The Makerspace Team<br></div>".html_safe
          end
      ...
      flash[
        :alert
      ] = "There has been an error, please make sure the file type is one that is accepted. You can try again later or contact: uottawa.makerepo@gmail.com"
    end
  ...
  def destroy
    ...
    if @user.staff? || @print_order.approved.nil?
      ...
      flash[
        :alert
      ] = "You cannot delete this print order. Please send an email to makerspace@uottawa.ca if you have any questions and add the order ID in the subject of the email."
    end
    if @user.id == user_id
      redirect_to index_new_print_orders_path
    else
  ...
  def print_order_params
    params.require(:print_order).permit(
      ...
      :comments_for_staff,
      :grams_carbonfiber,
      :price_per_gram_carbonfiber,
      :price_per_gram_fiberglass,
      :grams_fiberglass,
      :payed,
      :picked_up,
      :pdf_form,
  ...
  def set_pricing
    prices =
      if (@user.identity == "undergrad") || (@user.identity == "grad") ||
           (@user.identity == "faculty_member")
  ...
  def create_update_quote
    params[:print_order][:sst] = "true" if params[:print_order][:material] ==
      "SST"

    if params[:print_order][:price_per_hour] &&
         params[:print_order][:material_cost] &&
         params[:print_order][:service_charge]
      params[:print_order][:quote] = params[:print_order][
        :service_charge
      ].to_f + params[:print_order][:price_per_hour].to_f +
        params[:print_order][:material_cost].to_f
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $expedited_price.to_f if @print_order.expedited == true
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $clean_part_price.to_f if @print_order.clean_part == true
    elsif params[:print_order][:price_per_hour] &&
          params[:print_order][:hours] && params[:print_order][:service_charge]
      params[:print_order][:quote] = params[:print_order][
        :service_charge
      ].to_f +
        (
          params[:print_order][:price_per_hour].to_f *
            params[:print_order][:hours].to_f
        )
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $expedited_price.to_f if @print_order.expedited
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $clean_part_price.to_f if @print_order.clean_part == true
    elsif params[:print_order][:price_per_gram] &&
          params[:print_order][:grams] && params[:print_order][:service_charge]
      if @print_order.material == "M2 Onyx"
        params[:print_order][:quote] = params[:print_order][
          :service_charge
        ].to_f +
          (
            params[:print_order][:grams].to_f *
              params[:print_order][:price_per_gram].to_f
          ) +
          (
            params[:print_order][:grams_fiberglass].to_f *
              params[:print_order][:price_per_gram_fiberglass].to_f
          ) +
          (
            params[:print_order][:grams_carbonfiber].to_f *
              params[:print_order][:price_per_gram_carbonfiber].to_f
          )
      else
        params[:print_order][:quote] = params[:print_order][
          :service_charge
      ...
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $expedited_price.to_f if @print_order.expedited
      params[:print_order][:quote] = params[:print_order][:quote].to_f +
        $clean_part_price.to_f if (
        (@print_order.clean_part == true) &&
          (params[:print_order][:clean_part] == true)
      )
    end
  end
end
