class RfidController < SessionsController
  ...
  def check_session
    rfid = Rfid.find_by(card_number: params[:rfid])
    user = User.find_by(id: rfid.user_id)
    active_session = user.lab_sessions.where("sign_out_time > ?", Time.now)
    if active_session.present?
      active_session.update_all(sign_out_time: Time.now)
    else
      sign_in = Time.now
      sign_out = sign_in + 3.hours
      new_session = user.lab_sessions.new(sign_in_time: sign_in, sign_out_time: sign_out)
      new_session.save
    end
  end
end
