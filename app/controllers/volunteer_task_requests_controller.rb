...
class VolunteerTaskRequestsController < ApplicationController
  ...
  def search_pending
    ...
    if params[:search_pending].present?
      ...
      @pending_volunteer_task_requests = @volunteer_task_requests.not_processed.includes([:volunteer_task, :user]).joins(:volunteer_task, :user).order(created_at: :desc).paginate(page: params[:page], per_page: 15)
    end
  ...
  def search_processed
    ...
    if params[:search_processed].present?
      ...
      @processed_volunteer_task_requests = @volunteer_task_requests.processed.includes([:volunteer_task, :user]).joins(:volunteer_task, :user).order(created_at: :desc).paginate(page: params[:page], per_page: 15)
    end
    respond_to do |format|
      format.js
    end
