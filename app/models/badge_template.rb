...
class BadgeTemplate < ApplicationRecord
  ...
  def acclaim_api_get_badge_template
    response = Excon.get("#{Rails.application.credentials[Rails.env.to_sym][:acclaim][:url]}/v1/organizations/#{Rails.application.credentials[Rails.env.to_sym][:acclaim][:organisation]}/badge_templates/" + acclaim_template_id,
                 user: Rails.application.credentials[Rails.env.to_sym][:acclaim][:api],
                 password: '',
                 headers: { 'Content-type' => 'application/json' })
    JSON.parse(response.body)
