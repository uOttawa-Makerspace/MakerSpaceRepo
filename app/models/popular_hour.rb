      ...
      (0..6).each do |weekday|
        ...
        (0..23).each do |hour|
          if (start_date..end_date).count { |date| weekday == date.wday } == 0
            ...
            data[space.id][weekday][hour] = (
              s.where("extract('hour' from created_at) = ?", hour).count.to_f /
                (start_date..end_date).count { |date| weekday == date.wday }
            ).round(2)
          end
        end
      end
