...
namespace :popular_hours do
  ...
  task calculate_popular_hours: :environment do
    ...
    Space.all.each do |space|
      ...
      lab_sessions_not_logged_out =
        space.lab_sessions.where("sign_out_time > ?", Time.zone.now)
      current_users =
        (lab_sessions_in_space + lab_sessions_not_logged_out).uniq.count
      next if current_users == 0 # The space is closed now. So, to avoid the mean being too close to zero.
      day = Time.new.wday
      hour = Time.new.hour
      popular_hour =
        PopularHour.find_by(space_id: space.id, hour: hour, day: day)
      next if popular_hour.blank?
      prev_mean = popular_hour.mean
      new_count = popular_hour.count.next
      mean =
        ((popular_hour.count * popular_hour.mean) + current_users) / new_count
      popular_hour.update(
        count: new_count,
        mean: mean,
