...
class UsersControllerTest < ActionController::TestCase
  ...
  test 'user should be able to update profile with patch' do
    session[:user_id] = User.find_by(username: 'bob').id
    session[:expires_at] = 'Sat, 03 Jun 2030 05:01:41 UTC +00:00'
    patch :update, params: { username: 'bob', user: { gender: 'Female' } } # from male
  ...
  test 'user should be able to change password' do
    ...
    post :change_password, params: { username: 'bob.username', user: { old_password: 'Password1', password: 'Password2', password_confirmation: 'Password2' } }
  ...
  test "user can't change password if old password is wrong" do
    ...
    session[:expires_at] = 'Sat, 03 Jun 2030 05:01:41 UTC +00:00'
    @oldpass = User.find_by(username: 'bob').password
    post :change_password, params: { username: 'bob.username', user: { old_password: 'WrongOldPass1', password: 'Password2', password_confirmation: 'Password2' }, pword: 'Password1' }
    @newpass = User.find_by(username: 'bob').password
  ...
  test "user can't change password if passwords don't match" do
    ...
    session[:expires_at] = 'Sat, 03 Jun 2030 05:01:41 UTC +00:00'
    @oldpass = User.find_by(username: 'bob').password
    post :change_password, params: { username: 'bob.username', user: { old_password: 'Password1', password: 'Password2', password_confirmation: 'WrongConfirmationPass1' } }
    @newpass = User.find_by(username: 'bob').password
  ...
  test 'Regular users cannot view private repos of another user' do
    ...
    assert_not @repositories.include?(Repository.find(3))
  end
  ...
  test 'old valid user should be able to update profile' do
    ...
    patch :update, params: { username: 'mary', user: { name: 'Mary', gender: 'Female', identity: 'community_member' } }
    assert_equal 'Profile updated successfully.', flash[:notice]
  ...
  test 'Profile cannot be updated if input is invalid' do
    ...
    patch :update, params: { username: 'john', user: { name: 'John', gender: 'Male', identity: 'grad' } }
  ...
  test 'old valid user should be able to update profile with valid inputs' do
    ...
    patch :update, params: { username: 'john', user: { name: 'John', gender: 'Male', identity: 'grad', student_id: 9_876_543, faculty: 'arts', program: 'Honours BA in English', year_of_study: '3' } }
