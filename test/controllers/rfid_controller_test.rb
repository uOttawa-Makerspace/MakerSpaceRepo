...
class RfidControllerTest < ActionController::TestCase
  ...
  test "posting from a newly connected pi creates a new PiReader with no space_id" do
    ...
    assert PiReader.find_by(
             pi_mac_address: rfid.mac_address,
             space_id: nil
           ).present?
  end
  ...
  test "can sign in to a space" do
    ...
    post :card_number,
         params: {
           rfid: rfid.card_number,
           mac_address: raspi.pi_mac_address
         }

    lab_session =
      LabSession.where(user_id: rfid.user_id, space_id: raspi.space.id).last
  ...
  test "can sign out of a space" do
    ...
    assert rfid_status == "RFID sign out" # this is what the raspberry pi receives
    assert lab_session.present?
    assert lab_session.sign_out_time < Time.zone.now
    assert_not raspi.space.signed_in_users.include? rfid.user
  end
end
