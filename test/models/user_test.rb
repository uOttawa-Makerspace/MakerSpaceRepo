...
class UserTest < ActiveSupport::TestCase
  ...
  test "Allowed characters in the password" do
    ...
    assert user.valid?,
           "Your passwords must have one lowercase letter, one uppercase letter, one number and be eight characters long."
  ...
  test "Passwords matching" do
    user =
      User.create(
        password: "abABbc246dabc",
        password_confirmation: "abABbc246dabc"
      )
    assert_equal(
      user.password,
    ...
    user =
      User.create(
        password: "abABbc246dabc",
        password_confirmation: "ABabBC135DABC"
      )
    assert_not_equal(
      user.password,
      user.password_confirmation,
  ...
  test "validation of identity" do
    user = users(:bob)
    assert user.valid?,
           "Your identity is invalid. Must be one of: grad, undergrad, faculty_member, community_member, or unknown"
    ...
    assert user.invalid?,
           "Your identity is invalid. Must be one of: grad, undergrad, faculty_member, community_member"
  end
