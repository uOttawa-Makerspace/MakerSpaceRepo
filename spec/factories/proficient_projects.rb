FactoryBot.define do
  factory :proficient_project do
    ...
    trait :with_files do
      after(:create) do |pp|
        RepoFile.create(proficient_project_id: pp.id, file: Rack::Test::UploadedFile.new(Rails.root.join('spec/support/assets', 'RepoFile1.pdf'), 'application/pdf'))
        Photo.create(proficient_project_id: pp.id, image: Rack::Test::UploadedFile.new(Rails.root.join('spec/support/assets', 'avatar.png'), 'image/png'))
      end
    end
