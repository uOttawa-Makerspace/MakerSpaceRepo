...
RSpec.describe KeyCertification, type: :model do
  ...
  describe "Methods" do
    ...
    context "#generate_filename" do
      it "should return the right filename" do
        expect(@key_cert.generate_filename(1)).to eq("john-doe-WHMIS.pdf")
      end
    end
    ...
    context "#attach_pdf_file" do
      it "should attach the file" do
        ...
        @key_cert.attach_pdf_file(1, pdf_file)

        expect(@key_cert.send("pdf_file_1").blob.filename).to eq(
          "john-doe-WHMIS.pdf"
        )
      end
    end

    context "#get_key_certs_attached" do
      it "should get the number of key certifications attached" do
        pdf_file =
          fixture_file_upload(
            Rails.root.join("spec/support/assets", "RepoFile1.pdf"),
            "application/pdf"
          )
        (1..3).each { |i| @key_cert.send("pdf_file_#{i}").attach(pdf_file) }
        expect(@key_cert.get_key_certs_attached).to eq(3)
      end
    end
    ...
    context "#get_supervisor_certs_attached" do
      it "should return 0" do
        ...
        (1..3).each { |i| @key_cert.send("pdf_file_#{i}").attach(pdf_file) }
        expect(@key_cert.get_supervisor_certs_attached).to eq(0)
      end
      ...
      it "should return 1" do
        ...
        @key_cert.send(
          "pdf_file_#{KeyCertification::NUMBER_OF_STAFF_FILES + 1}"
        ).attach(pdf_file)
        expect(@key_cert.get_supervisor_certs_attached).to eq(1)
      end
    end
  end
end
