  ...
  describe 'POST /create' do
    context 'logged as admin' do
      it 'should create a staff availability' do
        sa_params = FactoryBot.attributes_for(:staff_availability, user_id: @staff.id)
        expect { post :create, params: {staff_availability: sa_params} }.to change(StaffAvailability, :count).by(1)
  ...
  describe 'PATCH /update' do
    context 'logged as staff' do
      it 'should update the staff_availability' do
        sa1 = create(:staff_availability, user_id: @staff.id)
        time = Time.now.utc
        patch :update, params: {id: sa1.id, staff_availability: {start_time: time} }
        expect(response).to redirect_to staff_availabilities_path
        expect(StaffAvailability.find(sa1.id).start_time.utc.strftime("%H:%M")).to eq(time.strftime("%H:%M"))
      end
    end
  end
  ...
  describe "DELETE /destroy" do
    context 'logged as admin' do
      it 'should destroy the staff availablity' do
        sa1 = create(:staff_availability, user_id: @staff.id)
        expect {  delete :destroy, params: {id: sa1.id} }.to change(StaffAvailability, :count).by(-1)
        expect(response).to redirect_to staff_availabilities_path
      end
    end
  end
