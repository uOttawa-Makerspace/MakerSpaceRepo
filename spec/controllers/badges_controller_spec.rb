  ...
  describe "#new_badge" do
    context "new badge" do
      it "should show the new page" do
        ...
        get :new_badge
        expect(response).to have_http_status(:success)
        expect(@controller.instance_variable_get(:@all_users).count).to eq(
          User.all.count
        )
        expect(
          @controller.instance_variable_get(:@users_with_badges).count
        ).to eq(User.all.joins(:badges).count)
        expect(
          @controller.instance_variable_get(:@badge_templates).count
        ).to eq(BadgeTemplate.joins(:proficient_projects).count)
      end
    end
  end
  ...
  describe "#revoke_badge" do
    context "revoke_badge" do
      ...
      it "should revoke the badge with params" do
        ...
        acclaim_response =
          Badge.acclaim_api_create_badge(
            User.find(user.id),
            @bt.acclaim_template_id
          )
        Badge.create(
          user_id: user.id,
          badge_template_id: @bt.id,
          acclaim_badge_id: JSON.parse(acclaim_response.body)["data"]["id"]
        )
        post :revoke_badge,
             params: {
               badge: {
                 acclaim_badge_id:
                   JSON.parse(acclaim_response.body)["data"]["id"],
                 user_id: user.id
               }
             }
      ...
      it "should not revoke the badge (error)" do
        ...
        post :revoke_badge, params: { order_item_id: OrderItem.last.id }
        expect(flash[:alert]).to include(
          "An error has occurred when removing the badge"
        )
        expect(response).to have_http_status(302)
      end
    end
  ...
  describe "#populate_badge_list" do
    context "populate_badge_list" do
      it "should get the badges from a user" do
        ...
        expect(JSON.parse(response.body)["badges"][0]["id"]).to eq(
          Badge.last.id
        )
      end
    end
