      ...
      it "should not create a shadowing shift (volunteer)" do
        ...
        expect {
          get :create_event,
              params: {
                space: "makerspace",
                datepicker_start: DateTime.now,
                datepicker_end: DateTime.now.tomorrow,
                user_id: user.id
              }
        }.to change(ShadowingHour, :count).by(0)
        expect(response).to redirect_to root_path
      end
  ...
  describe "#emails" do
    context "emails" do
      it "should get the list of emails" do
        ...
        get :emails
        expect(@controller.instance_variable_get(:@all_emails).count).to eq(
          total_volunteer + 3
        )
        expect(@controller.instance_variable_get(:@active_emails).count).to eq(
          2
        )
        expect(
          @controller.instance_variable_get(:@unactive_emails).count
        ).to eq(1)
        expect(response).to have_http_status(:success)
      end
    end
  ...
  describe "#volunteer_list" do
    context "volunteer_list" do
      it "should get the list of volunteers" do
        ...
        get :volunteer_list
        expect(
          @controller.instance_variable_get(:@active_volunteers).count
        ).to eq(2)
        expect(
          @controller.instance_variable_get(:@unactive_volunteers).count
        ).to eq(1)
        expect(response).to have_http_status(:success)
      end
    end
