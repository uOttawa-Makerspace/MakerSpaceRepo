...
RSpec.describe PriceRulesController, type: :controller do
  before(:all) do
    ...
    @price_rule =
      create(:price_rule, shopify_price_rule_id: @shopify_price_rule_id)
  end
  ...
  describe "POST /create" do
    context "create price rule" do
      it "should create a price rule" do
        price_rule_params =
          FactoryBot.attributes_for(:price_rule, shopify_price_rule_id: nil)
  ...
  describe "#check_discount_codes" do
    context "price rules with discount codes can't be updated" do
      it "should not update the price rule" do
        ...
        patch :update,
              params: {
                id: price_rule_with_discount_codes.id,
                price_rule: {
                  title: "8$ coupon",
                  value: 8,
                  cc: 80
        ...
        expect(PriceRule.find(price_rule_with_discount_codes.id).title).to eq(
          "5$ coupon"
        )
        expect(response).to redirect_to price_rules_path
  ...
  describe "DELETE /destroy" do
    context "destroy price rule" do
      it "should destroy a price rule" do
        expect { delete :destroy, params: { id: @price_rule.id } }.to change(
          PriceRule,
          :count
        ).by(-1)
        expect(response).to redirect_to price_rules_path
