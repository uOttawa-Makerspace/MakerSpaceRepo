  ...
  describe "#index" do
    context "index" do
      ...
      it "should show the index page" do
        ...
        expect(
          @controller.instance_variable_get(:@available_volunteer_tasks).count
        ).to eq(1)
      end
    end
  end
  ...
  describe "#create" do
    context "create" do
      ...
      it "should create a volunteer task" do
        ...
        expect {
          post :create, params: { volunteer_task: volunteer_task_params }
        }.to change(VolunteerTask, :count).by(1)
        expect(flash[:notice]).to eq(
          "You've successfully created a new Volunteer Task"
        )
      ...
      it "should create a volunteer task with certifications" do
        ...
        expect {
          post :create,
               params: {
                 volunteer_task: volunteer_task_params,
                 certifications_id: [t1.id, t2.id]
               }
        }.to change(VolunteerTask, :count).by(1)
        expect(
          RequireTraining.where(volunteer_task_id: VolunteerTask.last.id).count
        ).to eq(2)
        expect(flash[:notice]).to eq(
          "You've successfully created a new Volunteer Task"
        )
    ...
    it "should not let a regular user access this page" do
      ...
      expect {
        post :create, params: { volunteer_task: volunteer_task_params }
      }.to change(VolunteerTask, :count).by(0)
      expect(response).to redirect_to root_path
      expect(flash[:alert]).to eq("You cannot access this area.")
    end
  end
  ...
  describe "#my_tasks" do
    context "my_tasks" do
      it "should show the volunteer task" do
        ...
        expect(response).to have_http_status(:success)
        expect(
          @controller.instance_variable_get(:@your_volunteer_tasks).count
        ).to eq(1)
      end
    end
  end
  ...
  describe "#destroy" do
    context "destroy" do
      it "should destroy the volunteer task" do
        ...
        create(:volunteer_task)
        expect {
          delete :destroy, params: { id: VolunteerTask.last.id }
        }.to change(VolunteerTask, :count).by(-1)
        expect(response).to redirect_to volunteer_tasks_path
