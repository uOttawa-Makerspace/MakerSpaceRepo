  ...
  describe "#index" do
    context "index" do
      it "should show the index page" do
        ...
        get :index
        expect(
          @controller.instance_variable_get(:@user_volunteer_hours).count
        ).to eq(3)
      ...
      it "should create the hours" do
        ...
        hours_params =
          FactoryBot.attributes_for(
            ...
            :not_processed,
            user_id: user.id,
            volunteer_task_id: VolunteerTask.last.id
          )
        expect {
          post :create, params: { volunteer_hour: hours_params }
        }.to change(VolunteerHour, :count).by(1)
        expect(response).to redirect_to volunteer_hours_path
        expect(flash[:notice]).to eq(
          "You've successfully sent your volunteer working hours"
        )
      end
    end
  end
  ...
  describe "#destroy" do
    context "destroy" do
      it "should destroy the hours" do
        ...
        create(:volunteer_hour, :approved1, user_id: user.id)
        expect {
          delete :destroy, params: { id: VolunteerHour.last.id }
        }.to change(VolunteerHour, :count).by(-1)
        expect(
          response
        ).to redirect_to volunteer_hour_requests_volunteer_hours_path
      end
      ...
      it "should not destroy the hours" do
        ...
        expect {
          delete :destroy, params: { id: VolunteerHour.last.id }
        }.to change(VolunteerHour, :count).by(0)
        expect(response).to redirect_to volunteer_hours_path
      end
    end
  end
  ...
  describe "#volunteer_hour_requests" do
    context "volunteer_hour_requests" do
      it "should see the volunteer_hour_requests" do
        ...
        expect(response).to have_http_status(:success)
        expect(
          @controller.instance_variable_get(:@new_volunteer_hour_requests).count
        ).to eq(2)
        expect(
          @controller.instance_variable_get(:@old_volunteer_hour_requests).count
        ).to eq(3)
        expect(
          @controller.instance_variable_get(:@total_volunteer_hour_requests)
        ).to eq(2)
      end
    end
  end
  ...
  describe "#volunteer_hour_per_user" do
    context "volunteer_hour_per_user" do
      it "should see the volunteer_hour per user" do
        ...
        expect(response).to have_http_status(:success)
        expect(
          @controller.instance_variable_get(:@total_volunteer_hours_requested)
        ).to eq(50)
        expect(
          @controller.instance_variable_get(:@total_volunteer_hour_approved)
        ).to eq(20)
        expect(
          @controller.instance_variable_get(:@total_volunteer_hour_rejected)
        ).to eq(10)
      end
