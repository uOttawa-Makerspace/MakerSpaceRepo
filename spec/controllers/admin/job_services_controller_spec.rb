  ...
  describe 'POST /create' do
    context 'logged as admin' do
      it 'should create an job service and redirect' do
        params =  FactoryBot.attributes_for(:job_service_group).merge(job_service_group_id: create(:job_service_group).id)
        expect { post :create, params: {job_service: params} }.to change(JobService, :count).by(1)
  ...
  describe 'PATCH /update' do
    context 'logged as admin' do
      it 'should update the job service' do
        patch :update, params: {id: @job_service.id, job_service: {name: "abc123"} }
        expect(response).to redirect_to settings_job_orders_path
  ...
  describe "DELETE /destroy" do
    context 'logged as admin' do
      it 'should destroy the job service group' do
        expect { delete :destroy, params: {id: @job_service.id} }.to change(JobService, :count).by(-1)
        expect(response).to redirect_to settings_job_orders_path
      end
    end
  end
