  ...
  describe "GET /index" do
    context "logged as admin" do
      it "should return 200 response" do
        ...
        expect(@controller.instance_variable_get(:@courses).count).to eq(
          CourseName.count
        )
      end
    end
  ...
  describe "DELETE /destroy" do
    context "logged as admin" do
      it "should destroy the course" do
        expect { delete :destroy, params: { id: @course.id } }.to change(
          CourseName,
          :count
        ).by(-1)
        expect(@controller.instance_variable_get(:@changed_course)).to eq(
          @course
        )
        expect(response).to redirect_to admin_course_names_path
