  ...
  describe "GET /demotions" do
    context "logged as admin" do
      it "should return 200 response" do
        ...
        expect(@controller.instance_variable_get(:@demotions).count).to eq(
          Certification.inactive.count
        )
      end
    end
  ...
  describe "PATCH /update" do
    context "logged as admin" do
      it "should update the certification (demote) and return to the user profile" do
        ...
        expect(
          Certification.unscoped.find(@certification.id).demotion_staff
        ).to eq(@admin)
        expect(Certification.unscoped.find(@certification.id).active).to eq(
          false
        )
  ...
  describe "DELETE /destroy" do
    context "logged as admin" do
      it "should destroy the training" do
        expect { delete :destroy, params: { id: @certification.id } }.to change(
          Certification.unscoped,
          :count
        ).by(-1)
        expect(response).to redirect_to demotions_admin_certifications_path
        expect(flash[:notice]).to eq(
          "Certification deleted. This action can't be undone"
        )
      end
    end
  end
