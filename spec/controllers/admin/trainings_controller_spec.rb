  ...
  describe "GET /index" do
    context "logged as admin" do
      it "should return 200 response" do
        ...
        expect(@controller.instance_variable_get(:@trainings).count).to eq(
          Training.count
        )
      end
    end
  ...
  describe "GET /new" do
    context "logged as admin" do
      it "should return a 200" do
        get :new
        expect(response).to have_http_status(:success)
        expect(@controller.instance_variable_get(:@spaces).count).to eq(
          Space.count
        )
      end
    end
  ...
  describe "DELETE /destroy" do
    context "logged as admin" do
      it "should destroy the training" do
        expect { delete :destroy, params: { id: @training.id } }.to change(
          Training,
          :count
        ).by(-1)
        expect(@controller.instance_variable_get(:@changed_training)).to eq(
          @training
        )
        expect(response).to redirect_to admin_trainings_path
