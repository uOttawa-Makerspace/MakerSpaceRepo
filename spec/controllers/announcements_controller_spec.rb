  ...
  describe "POST /create" do
    context "logged as admin" do
      it "should create an announcement and redirect" do
        session[:user_id] = @admin.id
        announcement_params =
          FactoryBot.attributes_for(:announcement, :volunteer)
        expect {
          post :create, params: { announcement: announcement_params }
        }.to change(Announcement, :count).by(1)
        expect(flash[:notice]).to eq(
          "You've successfully created an announcement for Volunteer"
        )
  ...
  describe "PATCH /update" do
    context "logged as admin" do
      it "should update the announcement" do
        ...
        patch :update,
              params: {
                id: @announcement_volunteer.id,
                announcement: {
                  public_goal: "staff"
                }
              }
        expect(response).to redirect_to announcements_path
        expect(Announcement.find(@announcement_volunteer.id).public_goal).to eq(
          "staff"
        )
    ...
    context "logged as staff" do
      ...
      it "should redirect and not update" do
        ...
        expect(flash[:alert]).to eq("Please select another public goal")
        expect(response).to redirect_to new_announcement_path
        expect(Announcement.find(@announcement_admin.id).public_goal).to eq(
          "admin"
        )
      end
    end
  ...
  describe "DELETE /destroy" do
    context "logged as admin" do
      it "should destroy the announcement" do
        session[:user_id] = @admin.id
        expect {
          delete :destroy, params: { id: @announcement_volunteer.id }
        }.to change(Announcement, :count).by(-1)
        expect(response).to redirect_to announcements_path
    ...
    context "logged as staff" do
      ...
      it "should redirect and not delete" do
        session[:user_id] = @staff.id
        expect {
          delete :destroy, params: { id: @announcement_all.id }
        }.to change(Announcement, :count).by(0)
        expect(response).to redirect_to new_announcement_path
        expect(flash[:alert]).to eq("Please select another public goal")
      end
